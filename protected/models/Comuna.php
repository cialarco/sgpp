...
class Comuna extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('COM_ID, PRO_ID, COM_NOMBRE', 'required'),
			array('COM_ID, PRO_ID', 'numerical', 'integerOnly'=>true),
			array('COM_NOMBRE', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('COM_ID, PRO_ID, COM_NOMBRE', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'pRO' => array(self::BELONGS_TO, 'Provincia', 'PRO_ID'),
			'empresas' => array(self::HAS_MANY, 'Empresa', 'COM_ID'),
		);
	}
